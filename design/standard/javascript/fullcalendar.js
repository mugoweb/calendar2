						...
						if (buttonName == 'title') {
							...
							if (buttonClick) {
								...
								var button,
									...
									if( buttonName == 'basicWeekBackup' )
									{
										text = 'weekend';
									}

									if( buttonName == 'basicWeekTest' )
									{
										text = 'list';
									}
...
views.basicWeekTest = function(element, options)
{
	return new SimpleList( element, options );
};

views.basicWeekBackup = function(element, options) {
	return new Grid(element, options, {
		render: function(date, delta, height, fetchEvents) {
			if (delta) {
				addDays(date, delta * 7);
			...
			var visStart = this.visStart = cloneDate(
					this.start = addDays(cloneDate(date), -((date.getDay() - options.firstDay + 7 ) % 7) + 4 )
				),
				visEnd = this.visEnd = cloneDate(
					this.end = addDays(cloneDate(visStart), 3)
				);
			this.title = formatDates(
					visStart,
					addDays(cloneDate(visEnd) ),
					this.option('titleFormat'),
					options
				);
			this.renderGrid(
...
function SimpleList( element, options )
{
	this.element = element;
	this.testlist = null;

	view = $.extend(this, viewMethods );
	view.init( element, options );
	
	this.renderEvent = function( day_container, event )
	{
		day_container.find( "ol" ).append( "<li>" + event.title + "</li>" );
	};
	
	this.renderDayContainer = function( list, ts )
	{
		var day_container = $( "<li>" + formatDate( ts, 'MMMM d, yyyy' ) + "<ol></ol></li>" );

		day_container.appendTo( list );
	...
	this.renderEvents = function( events )
	{
		...
		{
			event_day = formatDate( events[i].start, 'yyyy-MMMM-d' );
			
			if( day != event_day )
			{
				day = event_day;
				
				day_container = view.renderDayContainer( view.testlist, events[i].start );
			}
			
			view.renderEvent( day_container, events[i] );
		}
	};
	
	this.rerenderEvents = function()
	{
		alert( '3' );
	};
	...
	this.defaultEventEnd = function()
	{
		alert( '5' );
	};
	
	this.visEventEnd = function()
	{
		alert( '6' );
	};

	this.render = function( date, delta, height, fetchEvents )
	{
		if( ! this.testlist )
		{
			this.testlist = $("<ol/>").appendTo( element );

			fetchEvents( this.renderEvents );
		}
	}
...
views.agendaWeekend = function(element, options) {
	return new Agenda(element, options, {
		render: function(date, delta, height, fetchEvents) {
			if (delta) {
				addDays(date, delta * 7);
